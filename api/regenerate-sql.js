...
// 재생성용 SQL 생성 프롬프트 생성 (고도화된 버전)
function createRegenerationPrompt(originalRequest, clarificationAnswers, relatedQueries, context, errorLearnings, commonPatterns, schemaInfo) {
  const { userQuery } = originalRequest;
  
  // 답변을 컨텍스트에 추가
  const answerText = clarificationAnswers.map(a => `${a.questionId}: ${a.answer}`).join('; ');
  const enhancedContext = {
    ...context,
    additionalInfo: (context?.additionalInfo || '') + ' 추가 정보: ' + answerText
  };
...
`;
  ...
  return `당신은 Dune Analytics SQL 쿼리 전문가입니다. 사용자의 자연어 요청과 추가 정보를 바탕으로 실제 존재하는 테이블과 컬럼을 사용하여 더 정확한 SQL 쿼리를 생성해주세요.
...
컨텍스트:
- 블록체인: ${enhancedContext?.blockchain || '지정 안됨'}
- 시간범위: ${enhancedContext?.timeframe || '지정 안됨'}
- 프로토콜: ${enhancedContext?.protocols?.join(', ') || '지정 안됨'}
- 추가 정보: ${enhancedContext?.additionalInfo || '없음'}
${schemaText}
...
4. 사용자의 추가 정보를 반영하여 더 정확한 쿼리를 생성하세요`;
}
...
module.exports = async (req, res) => {
  ...
  try {
    console.log('🔄 SQL 재생성 요청 받음');

    const { originalRequest, clarificationAnswers = [] } = req.body;

    if (!originalRequest || !originalRequest.userQuery) {
      return res.status(400).json({
        success: false,
        error: '원본 요청과 사용자 쿼리가 필요합니다.'
      });
    }
    ...
    if (!clarificationAnswers || clarificationAnswers.length === 0) {
      return res.status(400).json({
        success: false,
        error: '추가 질문 답변이 필요합니다.'
      });
    }

    console.log('📝 추가 답변 수:', clarificationAnswers.length);
    console.log('📝 답변 내용:', clarificationAnswers);

    // Supabase에서 관련 쿼리 및 오류 학습 데이터 검색
    const supabase = createSupabaseClient();
    let foundQueries = originalRequest.relatedQueries || [];
    let errorLearnings = [];
    let commonPatterns = [];
    ...
    if (supabase) {
      if (foundQueries.length === 0) {
        ...
        foundQueries = await findRelatedQueries(supabase, originalRequest.userQuery);
        console.log(`📊 ${foundQueries.length}개 관련 쿼리 발견`);
      }

      // 오류 학습 데이터 가져오기
      errorLearnings = await getErrorLearnings(supabase, originalRequest.userQuery);
    ...
    if (!claudeResponse) {
      ...
      let fallbackSQL = `-- AI가 재생성한 쿼리: ${originalRequest.userQuery}
-- 추가 정보: ${clarificationAnswers.map(a => a.answer).join('; ')}
SELECT 
  blockchain,
  project,
...
LIMIT 10`;
      ...
      const lowerQuery = originalRequest.userQuery.toLowerCase();
      if (lowerQuery.includes('nft') || lowerQuery.includes('collection')) {
        fallbackSQL = `-- AI가 재생성한 쿼리: ${originalRequest.userQuery}
-- 추가 정보: ${clarificationAnswers.map(a => a.answer).join('; ')}
SELECT 
  collection,
  SUM(amount_usd) as volume_usd
...
LIMIT 10`;
      ...
      const responseData = {
        ...
        explanation: "Claude API 연결 문제로 기본 SQL을 제공합니다. 추가 정보를 반영한 이더리움 DEX 거래량 분석 쿼리입니다.",
...
LIMIT 5`;
      ...
      result = {
        ...
        explanation: "추가 정보를 반영하여 SQL이 재생성되었습니다.",
        assumptions: ["기본 Dune Analytics 스키마 사용", "추가 정보 반영"],
        clarificationQuestions: undefined,
        confidence: sqlMatch ? 0.8 : 0.6,
        suggestedImprovements: [
          "Claude 응답 형식 개선 필요",
          "실제 토큰 심볼과 주소 확인 권장",
          "필요에 따라 필터 조건 수정"
